...
function Circle(x, y, rad) {
    ...
    (function() {
        ...
        self.opacity = settings.opacity[0];
    ...
    function interact() {
        ...
        if (Collision.isCollision(self.x, self.y, self.rad, mouse.x, mouse.y, 120) && self.opacity < settings.opacity[1]) {
            self.opacity += 0.05;
            self.opacity = Math.min(self.opacity, settings.opacity[1]);
        } else if (self.opacity > settings.opacity[0]) {
            self.opacity -= 0.05;
            self.opacity = Math.max(self.opacity, settings.opacity[0]);
        }
